...
async function addUserFieldsToFirestore(uid: string, userData: User): Promise<void> {
  try {
    console.log('Salvando no Firestore - UID:', uid, 'Dados:', userData);
    const docData = {
      ...userData,
      uid,
      nome: userData.nome,
    ...
    console.log('Documento a ser salvo:', docData);
    await setDoc(doc(db, 'Usuarios', uid), docData, { merge: true }); // merge: true para não sobrescrever campos existentes
...
async function getUserFromFirestore(uid: string): Promise<User | null> {
  try {
    console.log('Buscando usuário no Firestore - UID:', uid);
...
export const authService = {
  ...
  async registerUser(userData: User, senha: string): Promise<User> {
    try {
      console.log('Iniciando registro para:', userData.email, 'Tipo:', userData.tipo);
      const userCredential = await createUserWithEmailAndPassword(auth, userData.email, senha);
      const firebaseUser = userCredential.user;
      console.log('Usuário criado no Firebase Auth, UID:', firebaseUser.uid);

      await updateProfile(firebaseUser, {
        displayName: userData.nome
      });
      console.log('Profile atualizado com nome:', userData.nome);

      const userDataWithUid = { ...userData, uid: firebaseUser.uid };
      console.log('Salvando dados no Firestore:', userDataWithUid);
  ...
  async loginUser(email: string, senha: string): Promise<User> {
    try {
      ...
      const firebaseUser = userCredential.user;
      console.log('Login no Firebase realizado, UID:', firebaseUser.uid);

      // Recarregar dados do usuário para garantir que emailVerified esteja atualizado
      ...
      if (!userData) {
        ...
        userData = {
          ...
          celular: '',
          tipo: 'cliente', // Tipo padrão se não encontrado
          emailVerified: firebaseUser.emailVerified,
      ...
      console.log('Dados finais do usuário:', updatedUserData);
